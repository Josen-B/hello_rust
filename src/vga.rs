    ...
    pub fn write_string(&mut self, s: &str) {
        for byte in s.bytes() {
            match byte {
                ...
                _ => self.write_byte(0xfe), // 打印未知字符为■
            }
        }
    }
}
...
pub fn init_writer() {
    unsafe {
        WRITER = Some(Writer {
            ...
            buffer: &mut *(0xb8000 as *mut [[ScreenChar; BUFFER_WIDTH]; BUFFER_HEIGHT]),
        });
    }
}
...
// 简单的字符串写入函数，类似于原C版本的_strwrite
pub fn _strwrite(string: &str) {
    let vga_buffer = 0xb8000 as *mut u8;
    let mut p_strdst = vga_buffer;
    
    for byte in string.bytes() {
        unsafe {
            *p_strdst = byte;
            p_strdst = p_strdst.add(2);
        }
    }
}